...
def test_wetland_process_with_null_objectives(elan_processing, tmp_path):
    ...
    wetland_process_param["SINKS"] = str(test_data_dir / "wetland_process_steu_with_null_objectives_1.gpkg.zip")
    with pytest.raises(QgsProcessingException, match="These target values can't be NULL: COD"):
        elan_processing.run(test_wetland_process_alg, wetland_process_param)

    wetland_process_param["SINKS"] = str(test_data_dir / "wetland_process_steu_with_null_objectives_2.gpkg.zip")
...
def test_wetland_process_with_null_inflow_concentrations(elan_processing, tmp_path):
    """
    Test wetland process crashes when there is null values in inflow concentrations
    that trigger an error in wetlandoptimizer
    """
    ...
    wetland_process_param = {
        "CLIMATE": 0,
        "TREATMENT": str(tmp_path / "wetland_process_generated_output.gpkg"),
        "SINKS": str(test_data_dir / "wetland_process_steu_with_null_inflow_concentrations.gpkg.zip"),
        "AVAILABLE_SURFACE": None,
        "SINK_COORDS": "sink_coords",
        "STAGES_MAX": 3,
        "TSS_OBJ": "TSS_obj",
    ...
    with pytest.raises(
        QgsProcessingException,
        match=re.escape("An inflow concentration has a NULL value: [None, 0.5, None, 55.0, 789.0, 44.0, None]."),
    ):
        elan_processing.run(test_wetland_process_alg, wetland_process_param)
