...
def test_projection(elan_processing, tmp_path):
    ...
    projection_param = {
        "ROADS_INPUT_DATA": str(test_data_roads_buildings / "roads_buildings_output.gpkg.zip"),
        "BUILDINGS_INPUT_DATA": str(test_data_population / "population_reference_output.gpkg.zip"),
    ...
    elan_processing.run(test_projection_alg, projection_param)
    ...
    assert_same_layers(
        load_layer(ref_path, "projection_centroids_output"),
        load_layer(gen_centroids_path, "projection_centroids_output"),
    )
